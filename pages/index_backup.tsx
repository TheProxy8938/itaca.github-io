...
export default function Home() {
  ...
  const submitChatForm = async () => {
    const requiredFields = chatFormType === 'emergencia' 
      ? ['name', 'email', 'emergencyType', 'message']
      : ['name', 'email', 'message'];
    ...
    try {
      ...
      const confirmationText = chatFormType === 'emergencia'
        ? `🚨 **PROTOCOLO DE EMERGENCIA ACTIVADO**\n\n✅ **Solicitud ID:** EMG-${Date.now().toString().slice(-6)}\n\n⚡ **ACCIONES INMEDIATAS:**\n• Especialistas notificados AHORA\n• Te contactaremos en 15 minutos\n• Equipo de respuesta asignado\n\n📞 **Contacto inmediato:**\nWhatsApp: +52 442 186 7170\nEmail: ${chatFormData.email}\n${chatFormData.phone ? `Teléfono: ${chatFormData.phone}` : ''}\n\n🔄 **Seguimiento cada 30 minutos hasta resolución completa**`
      ...
      const confirmationMessage = {
        ...
        isUser: false,
        timestamp: new Date(),
        quickReplies: chatFormType === 'emergencia' 
          ? ['Estado de Emergencia', 'Contacto Directo', 'Nueva Consulta']
              ...
              <div className="p-4 bg-gradient-to-r from-blue-600 via-purple-600 to-indigo-700 text-white relative overflow-hidden">
                ...
                <div className="relative z-10">
                  ...
                  <motion.button
                    ...
                    whileTap={{ scale: 0.98 }}
                    onClick={() => handleQuickReply('🚨 EMERGENCIA')}
                    className="w-full bg-red-500 hover:bg-red-600 text-white py-2.5 px-4 rounded-lg font-semibold flex items-center justify-center space-x-2 shadow-lg transition-colors"
                  >
                    <span>🚨</span>
                    <span>EMERGENCIA EMPRESARIAL</span>
                  </motion.button>
                </div>
              </div>
                  ...
                  <AnimatePresence>
                    {showChatForm && (
                      <motion.div
                        ...
                        className={`sticky top-0 z-10 ${chatFormType === 'emergencia' ? 'bg-red-50 border-2 border-red-200' : 'bg-blue-50 border-2 border-blue-200'} rounded-xl p-4 mb-4 shadow-lg`}
                      >
                        <div className="flex items-center justify-between mb-4">
                          <h4 className={`font-bold ${chatFormType === 'emergencia' ? 'text-red-800' : 'text-blue-800'} flex items-center space-x-2`}>
                            <span>{chatFormType === 'emergencia' ? '🚨' : '📋'}</span>
                            <span>
                              {chatFormType === 'emergencia' ? 'Formulario de Emergencia' : 'Formulario de Cotización'}
                            </span>
                          </h4>
                          <button 
                            onClick={() => {
                            ...
                            className={`${chatFormType === 'emergencia' ? 'text-red-600 hover:text-red-800' : 'text-blue-600 hover:text-blue-800'} font-bold`}
                          >
                            ✕
                          </button>
                        </div>
                        ...
                        <div className="space-y-3">
                          <input
                            ...
                            className={`w-full p-3 border ${chatFormType === 'emergencia' ? 'border-red-300 focus:border-red-500' : 'border-blue-300 focus:border-blue-500'} rounded-lg focus:outline-none`}
                          />
                          ...
                          {chatFormType === 'emergencia' ? (
                            <select
                              value={chatFormData.emergencyType}
                              onChange={(e) => setChatFormData(prev => ({ ...prev, emergencyType: e.target.value }))}
                              className="w-full p-3 border border-red-300 focus:border-red-500 rounded-lg focus:outline-none"
                            >
                              <option value="">Tipo de emergencia *</option>
                              <option value="sitio_caido">Sitio web caído</option>
                              <option value="hackeo">Hackeo o seguridad</option>
                              <option value="crisis_reputacion">Crisis de reputación</option>
                              <option value="campana_problematica">Campaña problemática</option>
                              <option value="perdida_datos">Pérdida de datos</option>
                          ...
                          <textarea
                            placeholder={chatFormType === 'emergencia' ? "Describe la emergencia y su urgencia *" : "Describe tu proyecto o consulta *"}
                            value={chatFormData.message}
                            onChange={(e) => setChatFormData(prev => ({ ...prev, message: e.target.value }))}
                            className={`w-full p-3 border ${chatFormType === 'emergencia' ? 'border-red-300 focus:border-red-500' : 'border-blue-300 focus:border-blue-500'} rounded-lg focus:outline-none h-20 resize-none`}
                          />
                          ...
                          <motion.button
                            ...
                            } text-white py-3 rounded-lg font-semibold flex items-center justify-center space-x-2 shadow-lg transition-colors`}
                          >
                            <span>{chatFormType === 'emergencia' ? '🚨' : '📤'}</span>
                            <span>
                              {chatFormType === 'emergencia' ? 'ACTIVAR PROTOCOLO DE EMERGENCIA' : 'Enviar Solicitud'}
                            </span>
                          </motion.button>
